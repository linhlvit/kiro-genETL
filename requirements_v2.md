...
1. WHEN một File_Mapping tầng Silver → Gold được cung cấp, THE Mapping_Validator SHALL kiểm tra tất cả source tables đã tồn tại dưới dạng dbt_Model hoặc source declaration ở tầng Silver.
2. WHEN một File_Mapping tầng Bronze → Silver được cung cấp, THE Mapping_Validator SHALL kiểm tra tất cả source tables đã được khai báo trong source configuration của tầng Bronze.
3. IF một source table trong File_Mapping chưa được khai báo ở layer trước, THEN THE Mapping_Validator SHALL báo lỗi và liệt kê danh sách source tables bị thiếu kèm layer yêu cầu.
4. WHEN một File_Mapping tầng Silver → Gold phụ thuộc vào job Bronze → Silver, THE Mapping_Validator SHALL xác minh job Bronze → Silver tương ứng đã tồn tại trong Dependency_DAG.
5. IF một job phụ thuộc (prerequisite job) chưa tồn tại, THEN THE Mapping_Validator SHALL cảnh báo và liệt kê danh sách job cần được tạo trước khi chạy job hiện tại.
6. THE Mapping_Validator SHALL thực hiện xác minh trước khi Generator bắt đầu sinh dbt_Model, đảm bảo chỉ các File_Mapping hợp lệ về workflow mới được xử lý tiếp.

### Yêu cầu 3: Sinh dbt Model theo pattern DIRECT_MAP
...
### Yêu cầu 4: Sinh dbt Model theo pattern CAST
...
### Yêu cầu 5: Quản lý danh mục Transformation Rule
...
### Yêu cầu 6: Sinh dbt Model theo pattern HARDCODE
...
### Yêu cầu 7: Sinh dbt Model theo pattern UNPIVOT
...
### Yêu cầu 8: Sinh Filter Condition
...
### Yêu cầu 14: Xử lý thay đổi Mapping (Change Management)
...
### Yêu cầu 16: Báo cáo Review cho con người
