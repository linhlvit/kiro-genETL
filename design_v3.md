...
MappingValidator:
  ...
  validate_cte_pipeline(inputs: list[SourceEntry]) -> list[ValidationError]
  
ValidationResult:
  is_valid: bool
  missing_sources: list[MissingSource]
  missing_prerequisites: list[MissingPrerequisite]
  cte_errors: list[CTEValidationError]
...
- Kiểm tra CTE pipeline hợp lệ: `unpivot_cte` và `derived_cte` phải tham chiếu đến alias đã khai báo trước đó
...
*For any* MappingSpec và ProjectContext, tập hợp physical_table sources mà Validator báo thiếu SHALL bằng đúng tập hợp physical_table entries trong Input section mà không tồn tại trong context ở layer tương ứng (Bronze sources cho B→S, Silver models/sources cho S→G).
...
*For any* MappingSpec hợp lệ, mỗi `unpivot_cte` và `derived_cte` trong Input section SHALL tham chiếu đến một alias đã được khai báo bởi một SourceEntry có index nhỏ hơn. Validator SHALL phát hiện và báo lỗi khi tham chiếu đến alias chưa khai báo.

**Validates: Requirements 1.1, 1.2 (CTE pipeline validation)**
...
|-----------|--------|--------|---------|
| `ParseError` | CSV Parser | File CSV sai định dạng, thiếu section bắt buộc, sai source_type, unpivot format lỗi | Trả về lỗi với vị trí (dòng/cột) và loại lỗi. Dừng xử lý file đó. |
| `CTEValidationError` | Mapping Validator | CTE tham chiếu alias chưa khai báo, circular CTE dependency | Trả về lỗi chỉ rõ alias bị thiếu và CTE tham chiếu. |
...
| `GenerationError` | Generator Engine | Thiếu tham số (CAST thiếu precision/format), transformation không parse được | Trả về lỗi chỉ rõ trường và tham số thiếu. |
| `UnpivotParseError` | CTE Pipeline Builder | Unpivot select_fields format không hợp lệ (thiếu key field, sai separator) | Trả về lỗi chỉ rõ vị trí lỗi trong select_fields string. |
...
3. **Validation trước Generation**: Mapping Validator (bao gồm CTE pipeline validation) chạy trước Generator Engine. Chỉ các mapping hợp lệ về workflow mới được chuyển sang giai đoạn sinh code.
...
5. **Warning vs Error**: Lỗi nghiêm trọng (ParseError, CatalogError, CTEValidationError) ngăn sinh model. Cảnh báo (missing prerequisite, downstream impact) được ghi nhận nhưng không ngăn sinh model.

### Error Response Format

```
ErrorResponse:
  error_type: str          # Loại lỗi (ParseError, CTEValidationError, ...)
  message: str             # Mô tả lỗi dễ đọc
  location: Optional[ErrorLocation]  # Vị trí lỗi trong file
  context: dict            # Thông tin bổ sung (tên mapping, tên rule, alias, ...)
...
| Property 3 | Mapping Validator | Missing physical_table source detection across layers |
| Property 4 | Mapping Validator | Missing prerequisite detection |
| Property 5 | Mapping Validator | CTE pipeline dependency validation |
