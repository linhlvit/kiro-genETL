...
### 12. Spark Test Generator

Tự động sinh file pytest cho mỗi dbt model, kiểm chứng SQL sinh ra có thể chạy được trên PySpark local với fake data. Sử dụng hạ tầng test Spark hiện có (`conftest.py`, `sql_test_helper.py`, `fake_data_factory.py`).

```
SparkTestGenerator:
  generate_test(mapping: MappingSpec, sql_content: str, schema_dir: str) -> str
  generate_test_batch(models: list[GeneratedModel], schema_dir: str) -> dict[str, str]
```

**Hành vi:**
- Đọc Bronze schema files (`schemas/bronze/*.json`) để xác định cấu trúc bảng nguồn
- Sinh fake data rows (1-3 rows/table, với test date filter) dựa trên schema
- Copy helper files (`conftest.py`, `fake_data_factory.py`, `sql_test_helper.py`) vào thư mục test output để test files có thể chạy độc lập (self-contained)
- Sinh file pytest với:
  - Local import statements (`from fake_data_factory import register_fake_table`, `from sql_test_helper import prepare_sql`) — không phụ thuộc vào package `tests.spark`
  - Fake data constants cho từng bảng nguồn
  - SQL constant (đã xử lý: strip config block, replace Jinja vars, convert `::` thành `CAST()`, replace `hash_id` bằng `sha2(concat_ws('|', ...), 256)`, remove aggregate CTEs, fix UNION ALL alias references)
  - Expected columns list từ Mapping section
  - `setup` fixture để register temp views
  - 3 test methods: `test_sql_executes`, `test_correct_columns`, `test_has_rows`
  - Sử dụng `result._jdf.count()` thay vì `result.count()` để tránh lỗi Python worker
- Nếu Source_Schema_File không tồn tại cho bảng nguồn → sinh file test với TODO comment chỉ rõ bảng cần bổ sung schema
- Output path: `{output_dir}/tests/test_{model_name}_spark.py`
...
### Source_Schema_File — Schema bảng nguồn (Bronze)

Cùng cấu trúc với TargetSchemaFile nhưng dùng cho bảng nguồn Bronze. Được quản lý bởi source system owners, lưu tại `schemas/bronze/{TABLE_NAME}.json`. Dùng làm single source of truth cho cấu trúc bảng nguồn và fake data generation trong Spark integration tests.
...
**Quy ước đường dẫn:**
- Bronze schema files: `schemas/bronze/{TABLE_NAME}.json` (quản lý bởi source system owners)
- Silver schema files: `schemas/silver/{table_name}.json` (quản lý bởi Database Designers)
...
### Property 37: SELECT * detection

*For any* MappingSpec, nếu Mapping section rỗng (không có mapping entries) hoặc bất kỳ Input entry nào có select_fields="*", Validator SHALL trả về lỗi BLOCK loại SELECT_STAR. ValidationResult.is_valid SHALL là False.

**Validates: Requirements 2.9**

### Property 38: Spark Test File hợp lệ và đầy đủ test methods

*For any* MappingSpec có Source_Schema_Files tương ứng cho tất cả bảng nguồn, Spark_Test_File được sinh ra SHALL là Python hợp lệ về cú pháp (importable), và SHALL chứa đúng 3 test methods: `test_sql_executes`, `test_correct_columns`, `test_has_rows`. Expected columns trong test file SHALL khớp với danh sách target columns trong Mapping section.
...
| `SelectStarError` | Mapping Validator | Mapping section rỗng hoặc Input select_fields="*" | BLOCK. Không cho phép SELECT *. |
...
| Property 37 | Mapping Validator | SELECT * detection → BLOCK error |
| Property 38 | Spark Test Generator | Generated test file is valid Python with exactly 3 test methods |
...
Ngoài PBT và unit tests, hệ thống sử dụng Spark integration tests để kiểm chứng SQL sinh ra có thể chạy được trên Spark thực tế.

**Phương pháp:**
1. Đọc Bronze schema files (`schemas/bronze/*.json`) làm single source of truth cho cấu trúc bảng nguồn
2. Sinh fake data từ schema files bằng pure SQL (UNION ALL of SELECT literals — chạy hoàn toàn trong JVM, không cần Python worker), đăng ký thành temp views trên SparkSession
3. Chạy SQL sinh ra (sau khi strip dbt directives, thay thế schema refs, chuyển `::` thành `CAST()`, thay `hash_id` bằng `sha2(concat_ws('|', ...), 256)`, remove aggregate CTEs, fix UNION ALL alias references) trên PySpark 3.5 classic mode `master("local[1]")`
...
**Tech stack:** Python 3.13 + PySpark 3.5.5 (classic mode) + Java 17+ (khuyến nghị Java 21)

**Cấu trúc thư mục:**
```
tests/spark/                                 # Test viết tay (reference)
├── conftest.py                              # SparkSession fixture (PySpark classic, local[1])
├── sql_test_helper.py                       # Helpers: strip dbt directives, replace schema refs, convert :: to CAST(), replace hash_id
├── fake_data_factory.py                     # Đọc Bronze schema files, build Spark schemas, register temp views (pure SQL approach)
└── test_fund_management_company_spark.py    # 4 integration tests cho fund_management_company

output/silver/tests/                         # Test tự động sinh (self-contained)
├── conftest.py                              # Copy tự động từ tests/spark/
├── sql_test_helper.py                       # Copy tự động từ tests/spark/
├── fake_data_factory.py                     # Copy tự động từ tests/spark/
├── test_fund_management_company_spark.py    # Tự động sinh
└── test_involved_party_electronic_address_spark.py  # Tự động sinh
```

**Tự động sinh Spark Test File:**

SparkTestGenerator sử dụng hạ tầng test Spark hiện có để tự động sinh file pytest cho mỗi dbt model. File sinh ra là **self-contained** — generator copy `conftest.py`, `fake_data_factory.py`, `sql_test_helper.py` vào thư mục test output, và test files sử dụng local imports (`from fake_data_factory import register_fake_table`). Có thể chạy `pytest output/silver/tests/` trực tiếp mà không cần phụ thuộc vào package `tests.spark`.